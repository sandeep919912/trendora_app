  ...
  return (
    <>
      ...
      <div className="flex justify-between p-5 gap-10">
        {/* Sidebar */}
        <section className="w-1/4 rounded border max-h-max border-gray-800 p-2 flex flex-col gap-5">
          {linksName.map((items) => (
            <MotionNavLink
              key={items.route}
              to={items.route}
        ...
        {/* Main Content */}
        <section className="flex w-3/4 max-h-screen overflow-auto no-scrollbar scroll-auto rounded border border-gray-800">
          <Outlet />
        </section>
      </div>
    </>
